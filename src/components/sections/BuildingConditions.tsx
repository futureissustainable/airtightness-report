'use client';

import { useReportStore } from '@/store/reportStore';
import { Section, Input, Select, Card } from '@/components/ui';
import { Calculator } from '@phosphor-icons/react';

export default function BuildingConditions() {
  const {
    buildingConditions,
    updateBuildingConditions,
    volumeRows,
    addVolumeRow,
    removeVolumeRow,
    updateVolumeRow,
    getCalculatedResults,
  } = useReportStore();

  const { totalVolume } = getCalculatedResults();

  const calcMethodOptions = [
    { value: 'l_w', label: 'Length × Width' },
    { value: 'area', label: 'Direct Area' },
  ];

  return (
    <Section title="Building & Test Conditions" sectionNumber={2}>
      {/* Basic Conditions */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <Input
          label="Envelope Area (Aₑ)"
          type="number"
          step="0.01"
          unit="m²"
          value={buildingConditions.envelopeArea || ''}
          onChange={(e) =>
            updateBuildingConditions({ envelopeArea: parseFloat(e.target.value) || 0 })
          }
        />
        <Input
          label="Floor Area (Aբ)"
          type="number"
          step="0.01"
          unit="m²"
          value={buildingConditions.floorArea || ''}
          onChange={(e) =>
            updateBuildingConditions({ floorArea: parseFloat(e.target.value) || 0 })
          }
        />
        <Input
          label="Internal Temp."
          type="number"
          step="0.1"
          unit="°C"
          value={buildingConditions.internalTemp || ''}
          onChange={(e) =>
            updateBuildingConditions({ internalTemp: parseFloat(e.target.value) || 0 })
          }
        />
        <Input
          label="External Temp."
          type="number"
          step="0.1"
          unit="°C"
          value={buildingConditions.externalTemp || ''}
          onChange={(e) =>
            updateBuildingConditions({ externalTemp: parseFloat(e.target.value) || 0 })
          }
        />
      </div>

      {/* Volume Calculation */}
      <Card className="bg-[var(--color-surface)]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[var(--color-title)] flex items-center justify-center">
              <Calculator weight="bold" className="w-5 h-5 text-white" />
            </div>
            <div>
              <h4 className="text-[var(--color-title)]">Internal Volume Calculation</h4>
              <p className="text-sm">Add rooms/spaces to calculate total volume</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-[var(--color-muted)]">Total Volume (V)</p>
            <p className="text-2xl font-semibold text-[var(--color-title)]">
              {totalVolume.toFixed(2)} m³
            </p>
          </div>
        </div>

        {/* Volume Table */}
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border)]">
                <th className="text-left py-3 px-2 font-medium text-[var(--color-title)]">Space/Room</th>
                <th className="text-left py-3 px-2 font-medium text-[var(--color-title)]">Method</th>
                <th className="text-left py-3 px-2 font-medium text-[var(--color-title)]">Length (m)</th>
                <th className="text-left py-3 px-2 font-medium text-[var(--color-title)]">Width (m)</th>
                <th className="text-left py-3 px-2 font-medium text-[var(--color-title)]">Area (m²)</th>
                <th className="text-left py-3 px-2 font-medium text-[var(--color-title)]">Height (m)</th>
                <th className="text-right py-3 px-2 font-medium text-[var(--color-title)]">Sub-Volume</th>
              </tr>
            </thead>
            <tbody>
              {volumeRows.map((row) => (
                <tr key={row.id} className="border-b border-[var(--color-border-light)]">
                  <td className="py-2 px-2">
                    <Input
                      placeholder="e.g., Living Room"
                      value={row.name}
                      onChange={(e) => updateVolumeRow(row.id, { name: e.target.value })}
                      className="!py-2"
                    />
                  </td>
                  <td className="py-2 px-2">
                    <Select
                      options={calcMethodOptions}
                      value={row.method}
                      onChange={(e) =>
                        updateVolumeRow(row.id, { method: e.target.value as 'l_w' | 'area' })
                      }
                      className="!py-2"
                    />
                  </td>
                  <td className="py-2 px-2">
                    {row.method === 'l_w' ? (
                      <Input
                        type="number"
                        step="0.01"
                        value={row.length || ''}
                        onChange={(e) =>
                          updateVolumeRow(row.id, { length: parseFloat(e.target.value) || 0 })
                        }
                        className="!py-2"
                      />
                    ) : (
                      <span className="text-[var(--color-muted)]">—</span>
                    )}
                  </td>
                  <td className="py-2 px-2">
                    {row.method === 'l_w' ? (
                      <Input
                        type="number"
                        step="0.01"
                        value={row.width || ''}
                        onChange={(e) =>
                          updateVolumeRow(row.id, { width: parseFloat(e.target.value) || 0 })
                        }
                        className="!py-2"
                      />
                    ) : (
                      <span className="text-[var(--color-muted)]">—</span>
                    )}
                  </td>
                  <td className="py-2 px-2">
                    {row.method === 'area' ? (
                      <Input
                        type="number"
                        step="0.01"
                        value={row.area || ''}
                        onChange={(e) =>
                          updateVolumeRow(row.id, { area: parseFloat(e.target.value) || 0 })
                        }
                        className="!py-2"
                      />
                    ) : (
                      <span className="text-[var(--color-muted)]">—</span>
                    )}
                  </td>
                  <td className="py-2 px-2">
                    <Input
                      type="number"
                      step="0.01"
                      value={row.height || ''}
                      onChange={(e) =>
                        updateVolumeRow(row.id, { height: parseFloat(e.target.value) || 0 })
                      }
                      className="!py-2"
                    />
                  </td>
                  <td className="py-2 px-2 text-right font-mono text-[var(--color-title)]">
                    {row.subVolume.toFixed(2)} m³
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>

        {/* Add/Remove Buttons */}
        <div className="flex items-center justify-end gap-2 mt-4 pt-4 border-t border-[var(--color-border)]">
          <button
            onClick={removeVolumeRow}
            disabled={volumeRows.length <= 1}
            className="px-3 py-1.5 text-sm text-[var(--color-muted)] hover:text-[var(--color-error)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Remove Row
          </button>
          <button
            onClick={addVolumeRow}
            className="px-3 py-1.5 text-sm bg-[var(--color-title)] text-white rounded-lg hover:bg-[#2a2e36] transition-colors"
          >
            Add Row
          </button>
        </div>
      </Card>
    </Section>
  );
}
